...
struct mpsc_queue {
    ...
    alignas(CACHE_LINE_SIZE) size_t dequeue_pos;
};
...
mpsc_queue_t *init_mpsc_queue(size_t cap)
{
    ...
    mpsc_queue_t *q = (mpsc_queue_t *)aligned_alloc(CACHE_LINE_SIZE, sizeof(mpsc_queue_t));
    if (q == NULL) {
        return NULL;
    }
    memset(q, 0, sizeof(mpsc_queue_t));
